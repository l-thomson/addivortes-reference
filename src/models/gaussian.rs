...
//! one global σ² with the calibrated scaled-inverse-χ² draw. This is the
//! all-defaults path: the model file adds nothing to the engine's defaults,
//! which is itself the claim worth pinning — the engine's default sweep IS
//! this model.
...
/// Fit Gaussian AddiVortes: validate, then run the standard
/// burn-in/thinning loop over the default sweep.
pub(crate) fn fit(config: AddiVortesConfig, x: &Data, y: &[f64]) -> Result<FittedAddiVortes> {
    // The model's one rule: this file is the Gaussian model, whatever the
    // config carried. (The menu decides the family; the knobs stay free.)
    config
